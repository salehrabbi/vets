(function(factory) {
  ...
  function DateTimePicker(element, options) {
    var picker = {},
      ...
      NAV = CONSTS.NAV,
      I18N = CONSTS.I18N[options.language]
        ? CONSTS.I18N[options.language]
        : CONSTS.I18N["en"],
      cache = {
        showYear: null,
        showMonth: null
      ...
      currentDate =
        options.date instanceof Date ? new Date(options.date.getTime()) : null,
      ...
      _createDatePicker = function() {
        var $table = $(
          '<table cellspacing = "2px" cellpadding = "0" class="dt"/>'
        );
        ...
        //head - tools - title
        $table.$title = $('<td class="title" colspan="5"/>')
          .data("nav", NAV["title"])
          .appendTo($row);
        $table.$prevm = _createCell(
          $row,
        ...
        $row.appendTo($thead);
        //body
        var $tbody = $('<tbody onselectstart="return false;"/>').appendTo(
          $table
        );
        for (i = 6; i > 0; i--) {
          ...
          for (var j = 0; j < 7; ++j, date.setDate(iday + 1)) {
            ...
            if (!disabled) {
              if (current_month && iday == day) {
                cache.selectedDate &&
                  cache.selectedDate.removeClass("selected");
      ...
      _loadMonthData = function(table, date) {
        ...
        for (var i = 0; i < 12; i++) {
          ...
          if (
            (!utilsIsEmpty(minMonth) && i < minMonth) ||
            (!utilsIsEmpty(maxMonth) && i > maxMonth)
          ) {
          ...
          if (i < 10) {
            ...
            if (
              (!utilsIsEmpty(minYear) && years[i] < minYear) ||
              (!utilsIsEmpty(maxYear) && years[i] > maxYear)
            ) {
      ...
      _loadTimeData = function(table, date, viewmode) {
        ...
        if (
          viewmode == CONSTS.VIEWMODE.HM ||
          viewmode == CONSTS.VIEWMODE.YMDHM
        ) {
          date.setSeconds(0);
        }
      ...
      _createTimePicker = function(viewmode) {
        ...
        var $tbody = $("<tbody>").appendTo($table);
        $table.$h = $('<input type="number" min="0" max="23" maxlength="2"/>')
          .data("time", "h")
          .change(function() {
        ...
        $table.$m = $('<input type="number" min="0" max="59" maxlength="2"/>')
          .data("time", "m")
          .change(function() {
            var value = parseInt(this.value, 10);
        ...
        $table.$s = $('<input type="number" min="0" max="59" maxlength="2"/>')
          .data("time", "s")
          .change(function() {
            var value = parseInt(this.value, 10);
        ...
        $table.focus = $table.$m;
        var $add = $("<td/>")
          .append($('<i class="icon-datepicker-plus"/>'))
          .data("nav", NAV["plus"]);
        var $min = $("<td/>")
          .append($('<i class="icon-datepicker-minus"/>'))
          .data("nav", NAV["minus"]);
        var $row = $("<tr/>")
          .append($('<td rowspan="2"/>').text(I18N.TIME))
          ...
          if (type === "mouseover") {
            ...
            switch (navitype) {
              ...
              case NAV["title"]:
                ...
                _loadMonthData(
                  $monthtable,
                  new Date(cache.showYear, cache.showMonth)
                );
              ...
              case NAV["today"]:
                ...
                var today = new Date();
                if (
                  (options.startDate && today < options.startDate) ||
              ...
              case NAV["year"]:
                //choose one year
                cache.selectedYear &&
                  cache.selectedYear.removeClass("selected");
              ...
              case NAV["month"]:
                //choose one month
                cache.selectedMonth &&
                  cache.selectedMonth.removeClass("selected");
    ...
    var $wrapper = $("<div novalidate/>")
      .appendTo($el)
      .addClass(options.baseCls);
    ...
    picker.getValue = function() {
      ...
      if (
        date &&
        (options.viewMode === viewMode.YMD || options.viewMode === viewMode.YM)
      ) {
